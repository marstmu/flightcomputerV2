...
try:
    ...
    while True:
        try:
            # Create a 3-byte data packet
            data = bytearray([0x01, 0x02, 0x03])
            rfm9x.send(data)
            print("Sent data packet:", [hex(x) for x in data])
            print(f"Signal strength: {rfm9x.last_rssi} dB")
            time.sleep(2)  # Add delay between transmissions
    ...
    while True:
        try:
            ...
            if packet is not None:
                # Print raw bytes and signal data
                print("Received bytes:", [hex(x) for x in packet])
